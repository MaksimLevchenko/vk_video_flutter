...
class RutubeVideo extends StatefulWidget {
  const RutubeVideo({
    super.key,
  ...
  final RutubeVideoController? controller;
  ...
  @override
  State<RutubeVideo> createState() => _RutubeVideoState();
}

class _RutubeVideoState extends State<RutubeVideo> with AutomaticKeepAliveClientMixin {
  /// Timer to periodically check the state of the video.
  Timer? _stateCheckTimer;

  /// Controller for video playback state.
  RutubeVideoController? _playerController;
  ...
  void initState() {
    ...
    _playerController = widget.controller ?? RutubeVideoController();
  }
      ...
      child: Stack(
        children: [
          Center(
            ...
            child: widget.initialWidget ?? CircularProgressIndicator(color: Color(0xFFFFFFFF)),
          ),
          Positioned.fill(
            child: InAppWebView(
              ...
              initialData: InAppWebViewInitialData(
                ...
                data: Template.html.replaceAll("%ID%", widget.videoId)
                // .replaceAll(
                //     "%HD%", (widget.videoResolutionEnum?.index ?? _playerController!.getQuality.index).toString())
                // .replaceAll("%AUTO_PLAY%", widget.isAutoPlay ?? _playerController!.isAutoPlay ? "1" : "0")
                // .replaceAll(
                //     "%T%", (widget.videoStartTime ?? _playerController!.videoStartTime)
                ,
                mimeType: 'text/html',
                encoding: 'utf-8',
              ),
